...
mapview::mapview(traf_stat19) + mapview::mapview(unique_spatial2) + mapview::mapview(traf_stat19[traf_stat19$Accident_Severity == 2,])

# Accident Profile eg1 - Severity 3

eg = jan28_spatial_2[jan28_spatial_2$`Geographic Address` == "M1/3382B",]
eg$`Average Speed Lane 3` = as.integer(eg$`Average Speed Lane 3`)


p1 = ggplot(data = eg, aes(x = `Time GMT`)) +
  geom_point(mapping = aes(y = `Average Speed Lane 1`), colour = "blue")
p2 = ggplot(data = eg, aes(x = `Time GMT`)) +
  geom_point(mapping = aes(y = `Average Speed Lane 2`), colour = "red")
p3 = ggplot(data = eg, aes(x = `Time GMT`)) +
  geom_point(mapping = aes(y = `Average Speed Lane 3`), colour = "green")

egbuffer = st_buffer(eg, 500)

eg2 = traf_stat19[egbuffer,]
p4 = ggplot(data = eg2, aes(x = Time, y = Accident_Severity)) +
  geom_point()

grid.arrange(p1, p2, p3,p4, nrow = 1)

# Accident Profile eg3 - Severity 2 - Downstream

eg3 = jan28_spatial_2[jan28_spatial_2$`Geographic Address` == "M1/2516B",]
...
egbuffer = st_buffer(eg3, 500)
eg4 = traf_stat19[egbuffer,]
...
#p8 = ggplot(data = eg4, aes(x = Time, y = Accident_Severity)) +
#  geom_point()

grid.arrange(p5, p6, p7, p9, nrow = 1)

# Accident Profile eg3 - Severity 2 - Upstream
